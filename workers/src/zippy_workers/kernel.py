...
class SupabaseTaskSource:
    ...
    def complete(self, task_id: str, result: dict) -> None:
        self._client.rpc(
            "complete_agent_task", {"p_task_id": task_id, "p_result": result}
        ).execute()

    def fail(self, task_id: str, error: str) -> None:
        self._client.rpc("fail_agent_task", {"p_task_id": task_id, "p_error": error}).execute()

    def spend(self, agent: str, cents: int) -> None:
        self._client.rpc(
            "record_agent_spend", {"p_agent_name": agent, "p_amount_cents": cents}
        ).execute()
    ...
    def generate_quote(self, order_id: str) -> dict | None:
        ...
        if res.data and len(res.data) > 0:
            return (
                ...
                else {"total_amount": res.data[0][-1]}
            )
        return None
    ...
    def get_order(self, order_id: str) -> dict | None:
        res = self._client.table("orders").select("*").eq("order_id", order_id).limit(1).execute()
        if res.data and len(res.data) > 0:
    ...
    def upsert_document(
        ...
        res = self._client.rpc(
            "upsert_order_document",
            {
                "p_order_id": order_id,
                "p_doc_type": doc_type,
                "p_image_url": image_url,
                "p_ocr_raw_text": ocr_text,
    ...
    def mark_notification_sent(self, notification_id: str, external_id: str | None = None) -> None:
        self._client.rpc(
            "mark_notification_sent",
            {"p_notification_id": notification_id, "p_external_id": external_id},
        ).execute()

    def mark_notification_failed(self, notification_id: str, reason: str | None = None) -> None:
        self._client.rpc(
            "mark_notification_failed", {"p_notification_id": notification_id, "p_reason": reason}
        ).execute()
    ...
    def set_status(self, agent: str, status: str, reason: str | None = None) -> None:
        ...
        self._client.rpc(
            "set_agent_status", {"p_agent_name": agent, "p_status": status, "p_reason": reason}
        ).execute()
...
def default_tools_for(source: SupabaseTaskSource, settings: WorkerSettings) -> dict:
            ...
            for a in self.agents:
                try:
                    tools = self.tools.get(a) or (
                        default_tools_for(src, s)
                        if a in ("order_management", "communication", "resource_management")
                        else {}
                    )
