...
| M1-BACKUPS | M1 | Define backup, retention, monitoring, and restoration requirements | IN_PROGRESS | M1-PRD, M1-STAGING-ROUTE | Policy documentation only | RPO/RTO, schedule, retention, and future restore criteria | Owner and data custodian approval | Revise policy | Backup/restore execution belongs to later milestones; work has not begun |
...
`M1-BACKUPS` is the sole active task. `M1-DB-REQUIREMENTS` produced `docs/reports/M1_MVP_DATABASE_REQUIREMENTS.md` through static schema and ownership analysis. Backup-policy work has not begun and does not authorize backup creation, restore execution, database access, package changes, Compose implementation, provisioning, proxy changes, or production changes.
