...
| M1-STAGING-ROUTE | M1 | Design a staging-only deployment route and future production network constraints | IN_PROGRESS | M1-PRD, M1-DECISIONS, M1-MIGRATIONS | Design documentation only | Route, isolation, secret, rollback, and evidence review | Infrastructure owner approval | Revise/discard design | Completeness correction closed; no Compose implementation, provisioning, or proxy change; work has not begun |
...
`M1-STAGING-ROUTE` is the sole active task. `M1-MIGRATIONS` completed its owner-authorized expanded static inventory correction and identified no executable production chain. Staging-route work has not begun and does not authorize Compose implementation, provisioning, proxy changes, database execution, or production changes.
