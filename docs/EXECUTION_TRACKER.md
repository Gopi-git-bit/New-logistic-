...
| M1-DB-REQUIREMENTS | M1 | Define M2 database initialization and rollback requirements | IN_PROGRESS | M1-MIGRATIONS, M1-STAGING-ROUTE | Requirements documentation only | Environment gates, ordering, rollback, and future verification plan | Database owner approval | Revise requirements | Database creation and migration execution belong to M2; work has not begun |
...
`M1-DB-REQUIREMENTS` is the sole active task. `M1-LOCK-STRATEGY` produced `docs/reports/M1_DEPENDENCY_LOCK_STRATEGY.md` through static dependency analysis and a proposed locking policy. Database-requirements work has not begun and does not authorize package changes, database creation, migration execution, Compose implementation, provisioning, proxy changes, or production changes.
