...
| M1-EVIDENCE | M1 | Define fresh CI, security, contract, database, and restore evidence | IN_PROGRESS | M1-PRD, M1-MIGRATIONS, M1-LOCK-STRATEGY, M1-BACKUPS | Evidence-matrix documentation only | Required commands, environments, result fields, gate owners | Engineering/security owner approval | Revise matrix | Dependency-eligible and active, but not begun; no tests executed under planning authorization |
...
`M1-EVIDENCE` is the sole active task and has not begun. `M1-BACKUPS` is complete: Gopinathan approved the Minimal-Cost MVP policy and is the initial backup and encryption-key custodian. This tracker handoff does not authorize backup creation, restore execution, database access, package changes, Compose implementation, provisioning, proxy changes, or production changes.
