...
| M1-MIGRATIONS | M1 | Select canonical migration directory without moving files | IN_PROGRESS | M1-PRD, M1-DECISIONS | Inventory and manifest documentation only | Ordered manifest, checksums, target DB ownership | Database and application owner approval | Revise manifest | No migrations executed; work has not begun |
...
`M1-MIGRATIONS` is the sole active task. `M1-DECISIONS` recorded owner-approved MVP direction and technical decisions in `docs/DECISIONS.md`. Migration work is limited to inventory and an ordered manifest; it has not begun and does not authorize database execution, file moves, infrastructure changes, or production changes.
