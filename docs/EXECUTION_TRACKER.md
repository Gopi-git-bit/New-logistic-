...
- Current verdict: **BLOCKED FOR PRODUCTION — M3 DETERMINISTIC CORE COMPLETE IN DISPOSABLE TEST ENVIRONMENTS; M4 ACTIVE BUT NOT BEGUN**.
...
| M4-OPERATIONS-FINANCE | M4 | Payments, dispatch, POD and Odoo staging | IN_PROGRESS | M3-CORE | None currently authorized; implementation is explicitly not begun by this tracker transition | Sandbox payment, duplicate prevention, compliance, POD, Odoo draft/posting, and reconciliation evidence | Product, finance, and Odoo owner approval required | Sandbox reset and staged rollback | No live credentials or ledger mirroring; sole active task, explicitly unstarted |
...
`M3-CORE` is complete for its authorized disposable implementation scope, including the recorded hardening corrections (non-root API image, hardened disposable socket with scram-sha-256 local auth, cleanup-status handling, and output-redaction tests). Evidence covers typed order intake, database-resolved authorization, deterministic test pricing, transition-RPC enforcement, atomic idempotency, PostgreSQL task/outbox retry and recovery, exact dependency locks, a pinned non-root API image build, restricted-login integration tests, rollback and cleanup. `M4-OPERATIONS-FINANCE` is the sole active task and is explicitly not begun by this handoff; its authorized scope remains empty and product, finance, and Odoo owner approvals are still required before any M4 work. No production database mutation, live payment, deployment, Odoo/Paperclip/n8n integration, agent activation or infrastructure change occurred.
