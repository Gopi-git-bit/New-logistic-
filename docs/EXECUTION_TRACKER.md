...
| M1-DECISIONS | M1 | Draft architecture decisions as `PROPOSED` | IN_PROGRESS | M1-PRD, M1-RECONCILE | Documentation only | Proposed entries with rationale, alternatives, impact, rollback | Required to approve each decision | Remove/revise proposals | Must not mark proposals approved; execution has not begun |
...
`M1-DECISIONS` is the sole active task. `M1-RECONCILE` produced `docs/reports/M1_PRD_CONFLICT_MATRIX.md` without editing legacy sources. Decision drafting has not begun and this tracker handoff does not authorize implementation, infrastructure, database, service, or production changes.
