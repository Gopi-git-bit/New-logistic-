...
| M5-PAPERCLIP | M5 | Paperclip governance | IN_PROGRESS | M4-OPERATIONS-FINANCE, preserved immutable Paperclip source | None currently authorized; implementation is explicitly not begun by this tracker transition | Isolated schema, decision lock/HITL, grant, fail-closed, loop, budget, and audit evidence | Governance and owner approval required before any M5 work | Approved isolated service/database rollback | `/opt/paperclip` must not be modified or deployed until preserved/reviewed; sole active task, explicitly unstarted |
