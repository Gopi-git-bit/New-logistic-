...
| M1-OWNER-GATE | M1 | Review all M1 artifacts and authorize or reject implementation | IN_PROGRESS | All prior M1 tasks | Review and decision recording only | Explicit per-artifact decisions and exact implementation scope | Owner approval required | Keep M2 blocked and revise M1 artifacts | Sole active task, not begun; approval must name files/services/environments/commands |
...
`M1-OWNER-GATE` is the sole active task and has not begun or been approved. `M1-EVIDENCE` is complete with a documentation-only review that preserves all runtime, migration, staging, backup/restore, payment, and production limitations as `NOT VERIFIED`. This handoff does not authorize M2, database access, package changes, deployment, integration activation, infrastructure changes, or production changes.
