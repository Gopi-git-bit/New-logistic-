...
from zippy_workers.handlers import place_order, assign_driver, update_delivery_status
...
class FakeM6Db:
    ...
    def generate_quote(self, order_id: str) -> dict | None:
        return self.quotes.get(order_id, {
            "vehicle_class": "LCV", "rate_per_km": 25.0,
            "freight_amount": 3750.0, "toll_amount": 120.0,
            "loading_amount": 112.5, "tax_amount": 199.13,
...
def test_place_order_invalid_payment_plan():
    db = FakeM6Db()
    r = place_order(
        {"order_id": "o-2", "customer_id": "c-1",
         "payment_mode": "partial", "advance_amount": 100},
        db,
    )
...
def test_assign_driver_happy_path():
    db = FakeM6Db()
    db.orders["o-10"] = {
        "order_id": "o-10", "pickup_location": "SRID=4326;POINT(72.835 18.939)",
        "pickup_latitude": 18.939, "pickup_longitude": 72.835,
    }
    db.driver_matches = [
        {"user_id": "u-d1", "driver_id": "d-1", "driver_name": "Ravi Kumar",
         "rating": 4.5, "vehicle_id": "v-1", "vehicle_type": "LCV",
         "capacity_tons": 2.5, "distance_m": 1500.0, "score": 44.25},
    ]
...
def test_assign_driver_no_match():
    db = FakeM6Db()
    db.orders["o-11"] = {
        "order_id": "o-11", "pickup_location": "SRID=4326;POINT(72.835 18.939)",
    }
    db.driver_matches = []
...
def test_assign_driver_fallback_to_latlng():
    db = FakeM6Db()
    db.orders["o-12"] = {
        "order_id": "o-12", "pickup_location": None,
        "pickup_latitude": 18.94, "pickup_longitude": 72.84,
    }
    db.driver_matches = [
        {"user_id": "u-d2", "driver_id": "d-2", "driver_name": "Test Driver",
         "rating": 4.0, "vehicle_id": "v-2", "vehicle_type": "MCV",
         "capacity_tons": 9.0, "distance_m": 5000.0, "score": 35.0},
    ]
...
def test_assign_driver_no_pickup_at_all():
    db = FakeM6Db()
    db.orders["o-13"] = {
        "order_id": "o-13", "pickup_location": None,
...
def test_assign_driver_idempotent_already_assigned():
    ...
    db = AlreadyAssigned()
    db.orders["o-14"] = {
        "order_id": "o-14", "pickup_location": "SRID=4326;POINT(72.835 18.939)",
    }
    db.driver_matches = [
        {"user_id": "u-d3", "driver_id": "d-3", "driver_name": "Idem Driver",
         "rating": 4.2, "vehicle_id": "v-3", "vehicle_type": "LCV",
         "capacity_tons": 2.5, "distance_m": 2000.0, "score": 41.0},
    ]
